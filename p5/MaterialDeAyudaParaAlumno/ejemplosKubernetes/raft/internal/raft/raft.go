...
func NuevoNodo(nodos []rpctimeout.HostPort, yo int,
	...
	nr.TimerEleccion = time.NewTimer(time.Duration(rand.Intn(500)+1000) * time.Millisecond)
	nr.TimerLatido = time.NewTimer(150 * time.Millisecond)
...
func (nr *NodoRaft) PedirVoto(peticion *ArgsPeticionVoto,
	...
	nr.Logger.Println("DoyVoto",nr.E.VotedFor, nr.E.CommitIndex , peticion.LastLogIndex)
...
func (nr *NodoRaft) AppendEntries(args *ArgAppendEntries,
	...
	nr.TimerEleccion.Reset(time.Duration(rand.Intn(500)+1000) * time.Millisecond) // reseteamos tiempo de timeout para el latido
	...
	} else if len(nr.E.Log)-1 < args.PrevLogIndex || nr.E.Log[(args.PrevLogIndex)].Mandato != args.PrevLogTerm {
		// 2. Responda falso si el registro no contiene una entrada en prevLogIndex cuyo término coincida con prevLogTerm
		nr.Mux.Lock()
		results.Success = false
		results.Term = nr.E.CurrentTerm
		nr.Mux.Unlock()
...
func (nr *NodoRaft) ConvertirseEnSeguidor(mandato int) {
	...
	tiempo := time.Duration(rand.Intn(500)+1000) * time.Millisecond
...
func (nr *NodoRaft) ConvertirseEnLider(lastLogIndex int) {
	...
	nr.TimerLatido.Reset(150 * time.Millisecond)
	nr.Roll = LIDER
	nr.IdLider = nr.Yo
	nr.E.VotedFor = nr.Yo
...
func (nr *NodoRaft) ConvertirseEnCandidato() {
	...
	// solicitamos el voto
	for i := 0; i < len(nr.Nodos); i++ {
		go nr.mandarSpamVoto(i, &args, &reply)
	}
	nr.Mux.Lock()
...
// Enviamos la solicitud del voto hasta que nos responda el nodo, o bien hasta que nos convirtamos en lideres
func (nr *NodoRaft) mandarSpamVoto(nodo int, args *ArgsPeticionVoto,
