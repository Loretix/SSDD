 ...
 */
 ...
 //gorutina
 func worker(wg *sync.WaitGroup, envioPeticiones chan net.Conn){

	for conn := range envioPeticiones {
 ...
 func crearWorkerPool(numTrabajadores int, envioPeticiones chan net.Conn) {  
    var wg sync.WaitGroup
    for i := 0; i < numTrabajadores; i++ {
        wg.Add(1)
        go worker(&wg, envioPeticiones)
    }
    wg.Wait()
 ...
 func main() {
	 ...
	 //pool de gorutinas
	 numTrabajadores := 8
	 go crearWorkerPool(numTrabajadores, envioPeticiones)
